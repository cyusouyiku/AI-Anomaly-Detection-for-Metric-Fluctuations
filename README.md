...
客服对话数据：**多语种 Embedding → 聚类 → LLM 总结进线主题**。用于模型监控场景，对比 **固定 scenario 基准日** 与 **+1日（对比日）** 两期改签数据，发现不同对话主题导致的进线差异。

需求背景：因为每天的进线原因都不同，传统方法是直接跳转到页面人工去看客户和robot的对话内容，还要自己总结对话内容，速度慢效率低，因此本项目开发出来是用来AI自动检测客户进线指标异动的工具链路。
...
附：final_pipeline.py是最后版本，当初是为了部署到zeus后压缩到一个文件
